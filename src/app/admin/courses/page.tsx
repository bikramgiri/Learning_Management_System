...
import { ICourse } from "@/database/models/course.schema";
 ...
 function Courses(){
  ...
  const openUpdateModal = useCallback((course: ICourse) => {
  ...
  const filtredCourses = courses.filter(
    (course) =>
      ...
      formatDate(course.createdAt.toString()).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const currentItems = filtredCourses?.slice(indexOfFirstItem, indexOfLastItem) || [];
...
<div className="flex flex-col">
  <div className=" overflow-x-auto">
    {isModalOpen && <AddCourseModal closeModal={closeModal} categoryId={categoryId as string} />}
    {isModalOpen && selectedCourse && <EditCourseModal closeModal={closeModal} course={selectedCourse} categoryId={categoryId as string} />}
    <div className="min-w-full inline-block align-middle">
      <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4">
        ...
        <div className="flex justify-between">
          <input onChange={(e)=>setSearchTerm(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search courses" />
          <button onClick={openModal} className="cursor-pointer flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"> Add Course </button>
        </div>
      </div>
        ...
        <table className=" min-w-full rounded-xl">
          <thead className="border-b-2 border-gray-400">
            <tr className="bg-gray-50">
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl"> Course ID </th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            { currentItems.length > 0 ? (
              ...
              return(
              <tr key={course._id} className="bg-white transition-all duration-500 hover:bg-gray-50">
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {course._id}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {course?.title} </td>
              ...
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {course?.description}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {formatDate(course.createdAt.toString())}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  <button onClick={() => openUpdateModal(course)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                  ...
                  <button onClick={()=>deleteCorse(course._id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
                    <svg className="cursor-pointer h-5 w-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
