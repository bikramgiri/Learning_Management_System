...
import { deleteLesson, fetchLesson, fetchLessons } from "@/store/lesson/lessonSlice";
 ...
 function Lessons(){
  ...
  const filtredLessons = lessons.filter(
    (lesson) =>
      ...
      lesson._id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      lesson.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      lesson.videoUrl.toLowerCase().includes(searchTerm.toLowerCase()) ||
      formatDate(lesson.createdAt.toString()).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const totalPages = Math.ceil((filtredLessons?.length || 0) / itemsPerPage);

  // Get current items
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filtredLessons?.slice(indexOfFirstItem, indexOfLastItem) || [];
...
<div className="flex flex-col">
  <div className=" overflow-x-auto">
    ...
    <div className="min-w-full inline-block align-middle">
      <div className="flex relative text-gray-500 focus-within:text-gray-900 mt-0 mb-4 justify-between">
        <button onClick={()=>redirect('/admin/courses')} className="cursor-pointer p-2 rounded-md group transition-all dark:text-white duration-500  flex bg-indigo-600 item-center">
          Back to Courses
        </button>
        <input onChange={(e)=>setSearchTerm(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search lessons" />
        <div className="justify-end absolute inset-y-0 ml-180 flex items-center pl-3 pointer-events-none ">
          <svg className="w-5 h-5 cursor-pointer" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="flex justify-end ">
          ...
          <button onClick={openModal} className="cursor-pointer flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-full shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"> Add Lesson </button>
        </div>
      </div>
        ...
        <table className=" min-w-full rounded-xl">
          <thead className="border-b-2 border-gray-400">
            <tr className="bg-gray-50">
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl"> Lesson ID </th>
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Title </th>
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Video URL </th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            { currentItems.length > 0 ? (
              ...
              return(
              <tr key={lesson._id} className="bg-white transition-all duration-500 hover:bg-gray-50">
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {lesson._id}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {lesson?.title} </td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {lesson?.videoUrl} </td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {lesson?.description}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {formatDate(lesson.createdAt.toString())}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  <button className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                    <svg className="cursor-pointer h-5 w-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <button onClick={()=>deleteLeson(lesson._id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
                    <svg className="cursor-pointer h-5 w-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {/* <button onClick={()=>redirect(`/admin/courses/${course._id}/lessons`)} className="cursor-pointer p-1 rounded-md group transition-all duration-500  flex bg-indigo-600 item-center">
                    View
                  </button> */}
                </div>
              </td>
              ...
              <ul className="flex items-center space-x-2">
                <li>
                  <button
                    ...
                    className="cursor-pointer flex items-center justify-center w-10 h-10 text-gray-400 hover:text-white hover:bg-gray-700 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <svg
                      className="w-4 h-4"
                      aria-hidden="true"
