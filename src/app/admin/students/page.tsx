...
import { useSession } from "next-auth/react";
 ...
 function Students(){
  ...
  const {students, status} = useAppSelector((store) => store.students);
  ...
  const filtredStudents = students.filter(
    (student) =>
      ...
      formatDate(student.createdAt.toString()).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const totalPages = Math.ceil((filtredStudents?.length || 0) / itemsPerPage);

  // Get current items
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filtredStudents?.slice(indexOfFirstItem, indexOfLastItem) || [];
    ...
    <div className="min-w-full inline-block align-middle">
      <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4">
        ...
        <div className="flex justify-between">
          <input onChange={(e)=>setSearchTerm(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search students" />
        </div>
      </div>
        ...
        <table className=" min-w-full rounded-xl">
          <thead className="border-b-2 border-gray-400">
            <tr className="bg-gray-50">
              ...
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Profile Image </th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            { currentItems.length > 0 ? (
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  <div className="h-12 w-12 flex-shrink-0">
                    <Image className="h-12 w-12 rounded-full object-cover" width={40} height={40} src={student?.profileImage} alt="user Image" />
                  </div>
                  {/* <div className="ml-4">
              ...
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {formatDate(student.createdAt.toString())}</td>
              <td className=" p-5 ">
              ...
              <tr>
                {status === 'loading' ? (
                  <td className="p-5 text-center text-sm leading-6 font-medium text-gray-900 " colSpan={8}> Loading Studdent... </td>
                ) : (
                  <td className="p-5 text-center text-sm leading-6 font-medium text-gray-900 " colSpan={8}> No Students available! </td>
                )}
              </tr>
