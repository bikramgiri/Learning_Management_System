...
import {ICategory}  from "@/database/models/category.schema";
 ...
 function Categories(){
  ...
  const openUpdateModal = useCallback((category: ICategory) => {
  ...
  const filtredCategories = categories.filter(
    (category)=>
      ...
      formatDate(category.createdAt.toString()).toLowerCase().includes(searchTerm.toLowerCase())
      // formatDate(category.createdAt.toString()).includes(searchTerm.toString())
  );
  ...
  const totalPages = Math.ceil((filtredCategories?.length || 0) / itemsPerPage);

  // Get current items
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filtredCategories?.slice(indexOfFirstItem, indexOfLastItem) || [];
...
<div className="flex flex-col">
  <div className=" overflow-x-auto">
    ...
    {isModalOpen && selectedCategory && <EditModal closeModal={closeModal} category={selectedCategory} />}
        ...
        <table className=" min-w-full rounded-xl">
          ...
          <tbody className="divide-y divide-gray-300 ">
            { currentItems.length > 0 && currentItems.map((category)=>{
              ...
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {category.description}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {formatDate(category.createdAt.toString())}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  <button onClick={() => openUpdateModal(category)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                  ...
                  <button onClick={()=>deleteCatgry(category._id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
                    <svg className="cursor-pointer h-5 w-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
