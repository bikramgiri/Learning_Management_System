...
import { fetchEnrollements, deleteEnrollment } from "@/store/admin/enrollment/enrollmentSlice";

 function Enrollments(){
  const dispatch = useAppDispatch();
  const {enrollments, status} = useAppSelector((store) => store.enrollments);
  ...
  const formatDate = (dateString: string) => {
    ...
    const deleteEnrolment = (id:string) => {
  ...
  const filteredEnrollments = enrollments.filter(
    (enrollment) =>
      ...
      enrollment.whatsapp.toLowerCase().includes(searchTerm.toLowerCase()) ||
      formatDate(enrollment.enrolledAt.toString()).toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-w-full inline-block align-middle">
      <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4">
        ...
        <div className="flex justify-between">
          <input onChange={(e)=>setSearchTerm(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search enrollments" />
        </div>
      </div>
        ...
        <table className=" min-w-full rounded-xl">
          <thead className="border-b-2 border-gray-400">
            <tr className="bg-gray-50">
              ...
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Enrolled At </th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            { currentItems.length > 0 ? (
              ...
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {enrollment?.enrollStatus} </td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {enrollment?.whatsapp} </td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {formatDate(enrollment?.enrolledAt.toString())}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={()=>deleteEnrolment(enrollment._id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
                    <svg className="cursor-pointer h-5 w-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <tr>
                {status === 'loading' ? (
                  <td className="p-5 text-center text-sm leading-6 font-medium text-gray-900 " colSpan={8}> Loading enrollments... </td>
                ) : (
                  <td className="p-5 text-center text-sm leading-6 font-medium text-gray-900 " colSpan={8}> No enrollments available! </td>
                )}
              </tr>
